...
namespace generic_parser {
    ...
    class parser_facade {
        // Values are initialized in the order they appear in class, so 
        // generator and builder will both get valid pointers to this class.
        // This field will be passed as parameter to both builder and token_generator, 
        // builder will use names_stack to get tokens instead of names.
    ...
    public:
        template<typename... builder_args>
        parser_facade(
        ...
        // Auto will ignore const qualifier, references, etc. so this function returns decltype(auto).
        decltype(auto) get_builder_value() { return this->builder.get_value(); }

        builder_type& get_builder() { return this->builder; }
        std::vector<std::pair<std::string, token_type>>& get_names_stack() { return this->names_stack; }
        token_generator<token_type, context_key_type>& get_token_generator() { return this->generator; }
