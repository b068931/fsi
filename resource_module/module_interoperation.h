...
namespace interoperation {
	...
	public:
		...
		static std::size_t execution_module_on_container_creation() {
			static std::size_t index = get_module_part()->find_function_index(execution_module(), "on_container_creation");
			return index;
		}

		static std::size_t execution_module_on_thread_creation() {
			static std::size_t index = get_module_part()->find_function_index(execution_module(), "on_thread_creation");
			return index;
		}
		...
		static std::size_t program_loader_free_program() {
			static std::size_t index = get_module_part()->find_function_index(program_loader(), "free_program");
			return index;
		}
	};
}
