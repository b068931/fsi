...
private:
	...
	uint32_t function_jump_table_index;
	const runs_container::function_signature* fnc_signature;

	int32_t stack_address;
	void prologue() {
		...
		if (size != memory_layouts_builder::variable_size::UNKNOWN) {
			this->stack_address -= static_cast<uint8_t>(size);
		}
	}
	...
	void move_value_from_reg000_to_stack(uint8_t active_type, bool is_R) {
...
public:
	...
	virtual void visit(std::unique_ptr<function> fnc) override {
		if (!this->fnc_signature) {
			this->function_jump_table_index = static_cast<uint32_t>(this->get_function_table_index(fnc->get_id()));

			auto found_signature = this->get_file_info().function_signatures.find(fnc->get_id());
			if (found_signature == this->get_file_info().function_signatures.end()) {
				this->assert_statement(false, "Unknown function.", fnc->get_id());
				return;
			}
			...
			variable_imm<uint64_t> immediate_function_displacement{
				static_cast<uint64_t>(this->get_function_table_index(fnc->get_id()))
			};
	...
	virtual void build() override {
		...
		for (const auto& fnc_argument : this->fnc_signature->argument_types) {
			...
			if (arg_size != memory_layouts_builder::variable_size::UNKNOWN) {
				stack_to_allocate += static_cast<uint8_t>(arg_size);
			}
		}
