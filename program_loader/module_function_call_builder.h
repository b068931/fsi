...
public:
    ...
    void visit(std::unique_ptr<engine_module> engine_module) override {
    ...
    void visit(std::unique_ptr<specialized_variable> variable) override { //will be called before all other arguments
        if (this->is_second_time) {
            ...
            if (this->current_rbx_displacement != 0) {
                return; //ignogre if it's not a first argument to be processed
            }
            if (variable->get_id() != 0) {
    ...
    void visit(std::unique_ptr<signed_variable> variable) override {
        this->place_generic_variable(variable.release());
    }
