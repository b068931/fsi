...
#include <stdint.h> //uintX_t, intX_t types
#include <vector>
#include <map>
#include <cassert>
...
private:
	...
	void add_new_type_object(uint8_t type_bits) {
		uint8_t type_group = (type_bits >> 2) & 0b11;
		...
		if (type_group == 0b00) {
			...
			entity_id id = this->run.get_object<entity_id>();
			uint8_t real_type = this->function_memory_layout[id].second;

			if (real_type == 4) { //if used variable has POINTER type
				uint8_t dereferenced_variables_count = this->run.get_object<uint8_t>();
				std::vector<entity_id> dereference_indexes;
				for (uint8_t counter = 0; counter < dereferenced_variables_count; ++counter) {
			...
			std::map<uint8_t, void(instruction_builder::*)()> object_create_map{
				{ 0b0100, & instruction_builder::generic_create_imm<uint8_t> },
				{ 0b0101, &instruction_builder::generic_create_imm<uint16_t> },
				{ 0b0110, &instruction_builder::generic_create_imm<uint32_t> },
				{ 0b0111, &instruction_builder::generic_create_imm<uint64_t> },
	...
	void create_new_type(uint8_t type_bits) {
		this->instruction_types.push_back(type_bits);
		this->add_new_type_object(type_bits);
	}

	uint8_t find_type_index(uint8_t type_bits) {
		for (uint8_t index = 0, instruction_types_count = static_cast<uint8_t>(this->instruction_types.size()); index < instruction_types_count; ++index) {
	...
	void self_call_by_index(uint8_t index) {
	...
	void write_variable_relative_address(generic_variable* variable, int32_t additional_displacement) {
		this->write_bytes<int32_t>(
...
protected:
	...
	{
		uint8_t instruction_arguments_count = (instruction_prefix >> 4) & 0b1111;
		for (uint8_t counter = 0, type_bytes_count = instruction_arguments_count / 2; counter < type_bytes_count; ++counter) {
			uint8_t type_byte = run.get_object<uint8_t>();
			uint8_t first_type_bits = (type_byte >> 4) & 0b1111;
			uint8_t second_type_bits = type_byte & 0b1111;

			this->instruction_types.push_back(first_type_bits);
			this->instruction_types.push_back(second_type_bits);
		}

		for (uint8_t type_bits : this->instruction_types) {
			this->add_new_type_object(type_bits);
		}
	...
	void generate_stack_allocation_code(uint32_t size) {
		::generate_stack_allocation_code(this->translated_instruction_symbols, size);
	}
	void generate_stack_deallocation_code(uint32_t size) {
	...
	void save_type_to_program_stack(uint8_t active_type) {
	...
	uint8_t get_current_variable_type() const { return this->current_variable_type; }
	...
	void use_r8_on_reg(uint8_t opcode, bool is_dereference, uint8_t active_type, bool is_R = false, uint8_t reg = 0) {
		uint8_t rex = 0x41;
	...
	void use_r8_on_reg_with_two_opcodes(uint8_t opcode00, uint8_t opcode, bool is_dereference, uint8_t active_type, bool is_R = false, uint8_t reg = 0) {
		if (active_type == 0b00) {
	...
	void create_variable_instruction_with_two_opcodes(uint8_t opcode00, uint8_t opcode, bool rex_R, generic_variable* variable, uint8_t reg = 0, uint32_t additional_displacement = 0) {
	...
	void create_variable_instruction(uint8_t opcode, bool rex_R, generic_variable* variable, uint8_t reg = 0, uint32_t additional_displacement = 0) { //rax_or_r8 = rex_R
		uint8_t rex = 0;
	...
	void load_pointer_info(std::pair<int32_t, uint8_t> variable_info, uint32_t additional_displacement = 0) {
	...
	void accumulate_pointer_offset(dereferenced_pointer* pointer, uint32_t additional_displacement = 0) {
		this->zero_r8();
	...
	void add_base_address_to_pointer_dereference(dereferenced_pointer* pointer, uint32_t additional_displacement = 0) {
		...
		uint32_t additional_read_size = active_type_size - 1;
		if (additional_read_size > 0) {
			...
			this->translated_instruction_symbols.push_back('\xc0');
			this->write_bytes<uint32_t>(additional_read_size);
		}
	...
	uint8_t get_arguments_count() const { return static_cast<uint8_t>(this->instruction_types.size()); }
	bool check_if_active_types_match() const { //makes sense only for group00 group01 group10
		assert(this->instruction_types.size() >= 2);

		uint8_t sample = this->instruction_types[0] & 0b11;
		for (size_t index = 1, instruction_types_count = this->instruction_types.size(); index < instruction_types_count; ++index) {
			uint8_t current_type = this->instruction_types[index];
	...
	std::pair<int32_t, uint8_t> get_variable_info(entity_id id) {
	...
	std::pair<std::unique_ptr<char[]>, size_t>& get_string_info(entity_id id) {
	...
	void self_call_by_type(uint8_t type_bits) {
