...
void runs_container::modules_reader(run_reader<runs_container>::run run) { //every run has its own specific structure, so we need to use different functions for each type of run (see bytecode_translator.h)
	for (std::uint64_t counter = 0, modules_count = run.get_object<std::uint64_t>(); counter < modules_count; ++counter) {
		...
		std::size_t found_module = 
			interoperation::get_module_part()->find_module_index(module_name.c_str()); //now we try to find module's index inside engine_module mediator

		if (found_module != interoperation::get_module_part()->module_not_found) {
			...
			for (std::uint64_t functions_counter = 0; functions_counter < number_of_functions; ++functions_counter) {
				...
				std::size_t found_function = interoperation::get_module_part()->find_function_index(found_module, function_name.c_str());
				if (found_function != interoperation::get_module_part()->function_not_found) {
...
void runs_container::jump_points_reader(run_reader <runs_container>::run run) {
	for (generic_parser::file_position_type counter = 0, jump_points_count = run.get_run_size() / (sizeof(std::uint32_t) + sizeof(std::uint32_t) + sizeof(std::uint64_t)); counter < jump_points_count; ++counter) {
