...
namespace backend {
	...
	void fill_in_reg_array_entry(std::uint64_t entry_index, char* memory, std::uint64_t value) {
	...
	std::uintptr_t initialize_thread_stack(
		...
		if (initializer != nullptr) {
			...
			std::uintptr_t result = apply_initializer_on_thread_stack(thread_stack_memory, thread_stack_end, arguments);
